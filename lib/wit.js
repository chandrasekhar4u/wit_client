...
const Wit = function(token, actions, logger) {
  ...
  const makeCallback = (i, sessionId, message, context, cb) => {
    ...
    return (error, json) => {
      ...
      mg = mg + json.msg + '\n';
      l.log('mg message: ' + JSON.stringify(mg));

      if(typeof JSON.stringify(json.entities) !== 'undefined' && JSON.stringify(json.entities)){
      	//Add entities to reply message.
       	 mg = mg + JSON.stringify(json.entities) + '\n';
       	  l.log('entities in json format: ' + JSON.stringify(json.entities));
       }
